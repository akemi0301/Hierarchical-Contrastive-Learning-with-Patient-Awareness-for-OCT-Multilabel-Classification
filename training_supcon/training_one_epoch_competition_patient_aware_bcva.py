...
from visualization.tsne import tsne_visualize
...
def train_Compeition_Patient_Aware_BCVA(train_loader, model, criterion, optimizer, epoch, opt):
    """one epoch training"""
    model.train()
    ...
    # Assuming 'model' is a DataParallel-wrapped model
    actual_model = model.module  # Get the actual model inside DataParallel

    # Access layers from the actual model
    if opt.grad_visualize:
        if opt.backbone in ['swin_B_224_22k', 'swin_B_384_22k', 'swin_L_224_22k', 'swin_L_384_22k']:
            target_layers = [actual_model.layers[-1].blocks[-1].norm2]
        else:
            target_layers = [actual_model[0].body.body.layer4[-1]]                                                                                                      
            
    for idx, (path_1, path_2, images, bcva) in enumerate(train_loader):
    # for idx, (path, images, bcva,cst,eye_id,patient) in enumerate(train_loader):
        ...
        # if idx == 100:
        #     if opt.tsne_visualize:  
        #         tsne_visualize(train_loader, model, epoch, idx, opt)
        #     if opt.grad_visualize:    
        #         visualize(opt.grad_method, model, target_layers, img_paths, epoch, idx+1, opt.img_visualize)
        ...
        #         tsne_visualize(train_loader, model, epoch, idx, opt)
        #     if opt.grad_visualize:
        #         visualize(opt.grad_method, model, target_layers, path_1, epoch, idx+1, opt.img_visualize, idx_check_layer=1)
        #         visualize(opt.grad_method, model, target_layers, path_2, epoch, idx+1, opt.img_visualize, idx_check_layer=1)
