...
def train_Combined(train_loader, model, criterion, optimizer, epoch, opt):
    ...
    for idx, (img_paths, images, bcva,cst,eye_id,patient) in enumerate(train_loader):
        data_time.update(time.time() - end)
        # print(f"\n shape images before concat:{images[0].shape}")
        # print(idx, len(images))
        # print(int(len(train_loader)/2))
        ...
        if(opt.num_methods == 0):
            ...
            if (opt.method1 == 'HCL'):
                ...
                # print("line 275 checking input of loss")
                # print(f"features.shape:{features.shape}")
                # # print(f"labels1 - bcva:{labels1}")
                # print(f"labels1.shape:{labels1.shape}")
                # exit()
               
                loss= criterion(features,labels1)
                # print(f"loss in training one epoch:{loss}")
                # exit()
        ...
        if idx == int(len(train_loader)/2):
            if opt.tsne_visualize:  
                tsne_visualize(train_loader, model, epoch, idx, opt)
            if opt.grad_visualize:
                visualize(opt.grad_method, model, target_layers, img_paths, epoch, idx+1, opt.img_visualize, idx_check_layer=1)
