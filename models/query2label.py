...
from .query2label_models.backbone import build_backbone
from .query2label_models.transformer import build_transformer
from .query2label_models.utils.misc import clean_state_dict
...
class Query2Label(nn.Module):
    def __init__(self, backbone, transfomer, num_class):
        ...
        print("\nline 68 In query2label __init__")
    ...
    def forward(self, input):
        ...
        # print(f"self.input_proj(src).shape:{self.input_proj(src).shape}")
...
def build_q2l(args):
    ...
    if not args.keep_input_proj:
        print(f"ärgs.keep_input_proj={args.keep_input_proj}")
