...
# Written by Ze Liu
# Rewritten with Supervised Constrastive by Chau Truong
# --------------------------------------------------------
import torch
...
class SwinTransformer(nn.Module):
    ...
    def forward_SupCon(self, x):
        # print("Ïn forward supcon normalize")
        x = self.forward_features(x)
        x = self.avgpool(x).squeeze(-1).squeeze(-1)  # B C 1
        # import ipdb; ipdb.set_trace()
        x = self.head(x)
        x = F.normalize(x, dim=1)
        # print("ënd forward supcon normalize")
        return x
...
def build_swin_transformer(modelname, img_size, num_classes=1000):
    assert modelname in ['swin_B_224_22k', 'swin_B_384_22k', 'swin_L_224_22k', 'swin_L_384_22k', 'swin_T_224_22k']
    model_para_dict = {
        'swin_B_224_22k': dict(
            embed_dim=128,
            depths=[ 2, 2, 18, 2 ],
