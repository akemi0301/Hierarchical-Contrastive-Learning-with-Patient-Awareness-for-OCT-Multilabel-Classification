...
import models
import models.query2label_models.tresnet_v2
...
def get_model_path(modelname):
    ...
    pretrained_dir = './pretrained_imagenet'
    ...
    PTDICT = {
        ...
        'tresnetm_v2': 'tresnet_m_448.pth',
        'tresnetl_v2': 'tresnet_l_v2_miil_21k.pth',
    }
    # print("pretrained dir:")
    # print(os.path.join(pretrained_dir, PTDICT[modelname]))
    # if os.path.exists(os.path.join(pretrained_dir, PTDICT[modelname])):
...
class Backbone(BackboneBase):
    ...
    def __init__(self, name: str,
        ...
        if name in ['resnet18', 'resnet50', 'resnet34', 'resnet101']:
            ...
            backbone = getattr(models.query2label_models.tresnet_v2, name)(
                {'num_classes': 128}
            )
            backbone.forward = backbone.forward_SupCon
...
def build_backbone(args):
    ...
    if args.backbone in ['swin_B_224_22k', 'swin_B_384_22k', 'swin_L_224_22k', 'swin_L_384_22k']:
        ...
        if args.pretrained and args.ckpt == '':
            ...
            from collections import OrderedDict
            _tmp_st = OrderedDict({k:v for k, v in clean_state_dict(checkpoint).items() if 'head' not in k}) # del 
            _tmp_st_output = backbone.load_state_dict(_tmp_st, strict=False)
            # print(str(_tmp_st_output))
            # return backbone
        
        # if args.pretrained == False and args.ckpt!='':
        #     print("CHECKING!!!")

        #     checkpoint = torch.load(args.ckpt, map_location='cpu')['model']
        #     if 'model' in checkpoint:
        #         checkpoint = checkpoint['model']
        #     if 'state_dict' in checkpoint:
        #         checkpoint = checkpoint['state_dict']
        ...
        if args.with_transformer_head == True:
            print(f"Backbone {args.backbone} with transformer head")
            backbone.forward = backbone.forward_features
            bb_num_channels = backbone.embed_dim * 8
            del backbone.avgpool
            del backbone.head
        else:
            print(f"Backbone {args.backbone} NO transformer head")
            bb_num_channels = backbone.embed_dim * 8
            # print(f"bb_num_channels:{bb_num_channels}")
            # print(f"backbone.num_features:{backbone.num_features}")
            # exit()
            backbone.head = nn.Sequential(
                    nn.Linear(backbone.num_features, backbone.num_features), # num_features of swin_B_384_22k =  1024
        ...
        # backbone.head = nn.Sequential(
        #             nn.Linear(backbone.num_channels, backbone.num_channels), # num_channels of tresnet L v2 =  2048
        #             nn.ReLU(inplace=True),
        #             nn.Linear(backbone.num_channels, 128)
        #         )

        # backbone.forward = backbone.forward_SupCon

    if args.pretrained == False and args.ckpt!='':
            print(f"Load ckpt at:{args.ckpt}")
...
def build_backbone_constrastive(args):
    print("In build_backbone_constrastive")
    position_embedding = build_position_encoding(args)
    train_backbone = True
    if args.backbone in ['swin_B_224_22k', 'swin_B_384_22k', 'swin_L_224_22k', 'swin_L_384_22k',  'swin_T_224_22k']:
        ...
        print(f"Backbone {args.backbone} with projection head")
        ...
        backbone.head = nn.Sequential(
            nn.Linear(backbone.num_channels, backbone.num_channels), # num_channels of swin_B_384_22k =  1024
            nn.ReLU(inplace=True),
            nn.Linear(backbone.num_channels, 128)
            )
        backbone.forward = backbone.forward_SupCon
